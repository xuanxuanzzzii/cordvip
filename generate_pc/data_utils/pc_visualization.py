...
def visualize_and_create_video(pt_folder_path, pose_folder_path, ply_file_path, save_folder, num_frames=500, frame_delay=0.1):
    """
    ...
    for i in range(num_frames):
        ...
        save_data_to_file(save_folder, i, object_poses[i], sampled_pc_3d, np.asarray(second_object_pcd.points))
        if i == 1:
            vis.add_geometry(first_object_pcd)
            vis.add_geometry(second_object_pcd)  

        vis.update_geometry(first_object_pcd)
        vis.update_geometry(second_object_pcd)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ply_file_path', type=str, required=True)   
    parser.add_argument('--save_folder', type=str, required=True)  
    args = parser.parse_args()
    pt_folder_path = args.pt_folder_path
    pose_folder_path = args.pose_folder_path
    ply_file_path = args.ply_file_path
    ply_second_file_path = args.ply_file_path
    save_folder = args.save_folder
    visualize_and_create_video(pt_folder_path, pose_folder_path, ply_file_path, save_folder, num_frames=500, frame_delay=0.05)
