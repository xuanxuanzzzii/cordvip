...
class HandModel:
    ...
    def get_transformed_links_pc(self, q=None, links_pc=None):
        ...
        se3_cnt = torch.eye(4, device=q.device)

        all_pc_se3 = []
        # print(links_pc.items())
    ...
    def get_sampled_pc(self, q=None, num_points=512):
        ...
        """
        if q is None:
            q = self.get_initial_q()
        tm_gen1 = time.time()
        sampled_pc, sampled_transform = self.get_transformed_links_pc(q, self.vertices)
        tm_gen2 = time.time()
        sampled_pc, sampled_pc_index = farthest_point_sampling(sampled_pc, num_points)
        tm_gen3 = time.time()
        # print('latency_gen1',tm_gen2-tm_gen1)
        # print('latency_gen2',tm_gen3-tm_gen2)
        return sampled_pc, sampled_pc_index, sampled_transform
...
def create_hand_model(
    ...
    if robot_name == 'leaphand':
        # urdf_path = '/home/alan/project/c/c/DRO-Grasp/assets/ur5_leaphand/ur5_leaphand.urdf'
        # meshes_path = '/home/alan/project/c/c/DRO-Grasp/assets/ur5_leaphand/mesh'
        urdf_path = os.path.join(ROOT_DIR, 'assets/ur5_leaphand/ur5_leaphand.urdf')
        meshes_path = os.path.join(ROOT_DIR, 'assets/ur5_leaphand/mesh')
