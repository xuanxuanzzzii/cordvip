...
class RobotEnv:
    ...
    def apply_act(self, act_model):
        ...
        query_frequency = 30
        start = time.time()
        point_cloud, joint_state = self.get_observation()
        # print ("point_cloud:",point_cloud)
        while cnt < self.episode_steps:
            start = time.time()
            with torch.no_grad():
                if cnt % query_frequency == 0:
                    action = act_model.run_inference(point_cloud, joint_state)  
                else:
                    pass
    ...
    def get_observation(self):
        ...
        ckpt_dir = "../policy/act+3d/checkpoints/" # todo: task_name
        ...
        return point_cloud[:, :,:3], joint_state
...
class ACT:
    ...
    def run_inference(self, point_cloud, joint_state):
        print ("joint_state",joint_state.shape)
        print ("point_cloud",point_cloud.shape)
        device = torch.device("cuda")
        output = self.model(joint_state.to(device),point_cloud.to(device))
