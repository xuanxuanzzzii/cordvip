...
from CordViP.policy.CordViP_pretrain import CordViP_Pretrain
...
class TrainCordViPWorkspace:
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        # configure model
        self.model: CordViP_Pretrain = hydra.utils.instantiate(cfg.policy)

        self.ema_model: CordViP_Pretrain = None
    ...
    def checkpoint_save(self, save_path):
        os.makedirs(os.path.dirname(save_path), exist_ok=True)
        checkpoint = {
            'obs_encoder_state_dict': self.model.obs_encoder.state_dict(),
        }
    ...
    def run(self):
        ...
        # configure logging
        wandb.init(
            project='CordViP_Pretrain_new'+'_'+cfg.policy.pointnet_type+'_'+cfg.task.name+'_'+str(cfg.horizon)+'_'+str(cfg.n_obs_steps)+'_'+str(cfg.n_action_steps),  
        ...
        for local_epoch_idx in range(cfg.training.num_epochs):
            ...
            with tqdm.tqdm(train_dataloader, desc=f"Training epoch {self.epoch}", 
                ...
                for batch_idx, batch in enumerate(tepoch):
                    ...
                    # compute loss
                    t1_1 = time.time()
                    raw_loss = self.model.compute_loss_hand_and_arm(batch)
            ...
            if (self.epoch % cfg.training.val_every) == 0 and RUN_VALIDATION:
                with torch.no_grad():
                    ...
                    with tqdm.tqdm(val_dataloader, desc=f"Validation epoch {self.epoch}", 
                        ...
                        for batch_idx, batch in enumerate(tepoch):
                            batch = dict_apply(batch, lambda x: x.to(device, non_blocking=True))
                            loss = self.model.compute_loss_hand_and_arm(batch)
            ...
            if ((self.epoch + 1) % cfg.training.checkpoint_every) == 0 and cfg.checkpoint.save_ckpt:
                save_name = 'CordViP_Pretrain_new'+'_'+cfg.policy.pointnet_type+'_'+cfg.task.name + '_' + str(cfg.horizon)+'_'+str(cfg.n_obs_steps)+'_'+str(cfg.n_action_steps)
                print ("save_name:",save_name)
                self.checkpoint_save(f'checkpoints/{save_name}/{self.epoch + 1}.ckpt')
