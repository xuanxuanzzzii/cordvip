...
class PointNextEncoderXYZ(nn.Module):
    ...
    def __init__(self,
        ...
        # Set network hyperparameters
        self.blocks = [1, 1, 1]  # 将块数进一步减少
        self.strides = [1, 2, 1]  # 进一步减小stride的数量，减少步幅
        self.width = 8  # 进一步减小每层的通道数，减少每层的参数量
        self.in_channels = 3
        self.radius = 0.02  # 将半径减小到更小的值，进一步减少计算量
        self.radius_scaling = 2
        self.sa_layers = 1  # 继续保持1层，减少SetAbstraction模块的层数
        self.sa_use_res = True
        self.nsample = 8  # 进一步减少每个区域选择的点数
        self.nsample_scaling = 1
        self.expansion = 1  # 将MLP中的扩展倍数设为1，减少每个MLP层的参数量
        self.act_args = {'act': 'relu'}
        self.norm_args = {'norm': 'bn'}
        self.use_res = True
        self.num_posconvs = 1  # 将MLP中的位置卷积数量减少到1
        self.radii = self._to_full_list(self.radius, self.radius_scaling)
        self.nsample = self._to_full_list(self.nsample, self.nsample_scaling)
