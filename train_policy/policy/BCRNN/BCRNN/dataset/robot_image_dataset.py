...
import numpy as np
import copy
from BCRNN.common.pytorch_util import dict_apply
from BCRNN.common.replay_buffer import ReplayBuffer
from BCRNN.common.sampler import (
    SequenceSampler, get_val_mask, downsample_mask)
from BCRNN.model.common.normalizer import LinearNormalizer
from BCRNN.dataset.base_dataset import BaseImageDataset
from BCRNN.common.normalize_util import get_image_range_normalizer
import pdb
...
class RobotImageDataset(BaseImageDataset):
    def __init__(self,
        ...
        self.replay_buffer = ReplayBuffer.copy_from_path(
            ...
            keys=['img', 'joint_state', 'joint_action']
        )
    ...
    def _sample_to_data(self, sample):
        ...
        img = np.moveaxis(sample['img'],-1,1)/255

        data = {
            'obs': {
                'img': img, # T, 3, H, W
                'joint_state': joint_state, # T, D
            },
    ...
    def postprocess(self, samples, device):
        joint_state = samples['joint_state'].to(device, non_blocking=True)
        img = samples['img'].to(device, non_blocking=True) / 255.0
        joint_action = samples['joint_action'].to(device, non_blocking=True)
        return {
            'obs': {
                'img': img, # B, T, 3, H, W
                'joint_state': joint_state, # B, T, D
            },
