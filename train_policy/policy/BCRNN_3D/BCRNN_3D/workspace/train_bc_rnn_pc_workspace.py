...
import numpy as np
import shutil
from BCRNN_3D.workspace.base_workspace import BaseWorkspace
from BCRNN_3D.dataset.base_dataset import BaseImageDataset
from BCRNN_3D.common.checkpoint_util import TopKCheckpointManager
from BCRNN_3D.common.json_logger import JsonLogger
from BCRNN_3D.common.pytorch_util import dict_apply, optimizer_to


OmegaConf.register_new_resolver("eval", eval, replace=True)

class TrainRobomimicPointCloudWorkspace(BaseWorkspace):
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if ((self.epoch + 1) % cfg.training.checkpoint_every) == 0:
                    # checkpointing
                    # save_name = 'bc_rnn_pc' + '_' + (self.cfg.task.name + '_' + 
                    #     str(self.cfg.expert_data_num) + '_' + 
                    #     str(self.cfg.training.num_epochs) + '_' + 
                    #     str(self.cfg.horizon) + '_' + 
                    #     str(self.cfg.n_obs_steps) + '_' + 
                    #     str(self.cfg.n_action_steps) )
                    save_name = 'bc_rnn_pc' + '_' + self.cfg.task.name 
