...
class PointNet2EncoderXYZ(nn.Module):
    ...
    def __init__(self,
        ...
        self.sa1 = PointNetSetAbstraction(npoint=1, radius=0.2, nsample=512, in_channel=in_channels, mlp=[64, 128, 256], group_all=False)
        self.sa2 = PointNetSetAbstraction(npoint=None, radius=None, nsample=None, in_channel=256 + 3, mlp=[256, 512, out_channels], group_all=True)
    ...
    def forward(self, x):
        ...
        if self.normal_channel:
            ...
            norm = None
        x, norm = self.sa1(x, norm)
        x, norm = self.sa2(x, norm)
        x = norm.view(B, self.out_channels)
        return x